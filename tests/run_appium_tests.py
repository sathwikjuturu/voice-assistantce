...
def setup_logger():
    ...
    log_file = os.path.join(log_dir, "appium_test.log")
    
    logger = logging.getLogger("AppiumTests")
...
def run_tests():
    logger = setup_logger()
    logger.info("Starting Mobile Android Appium E2E Live Testing Pipeline")
    ...
    # 2. Configure Appium / Selenium Driver
    driver = None
    try:
        from appium import webdriver as appium_webdriver
        from appium.options.common import AppiumOptions
        
        logger.info("Connecting to Appium server on http://localhost:4723...")
        options = AppiumOptions()
        options.set_capability("platformName", "Android")
        options.set_capability("automationName", "UiAutomator2")
        options.set_capability("browserName", "Chrome")
        options.set_capability("deviceName", "Android Emulator")
        options.set_capability("headless", True)
        options.set_capability("noReset", True)
        
        driver = appium_webdriver.Remote("http://localhost:4723", options=options)
        logger.info("Appium Mobile Chrome driver initiated successfully.")
    except Exception as appium_ex:
        logger.warning(f"Appium connection failed: {appium_ex}")
        logger.info("Falling back to local Desktop Chrome driver for execution...")
        
        chrome_options = ChromeOptions()
        chrome_options.add_argument("--headless")
        chrome_options.add_argument("--no-sandbox")
        chrome_options.add_argument("--disable-dev-shm-usage")
        chrome_options.add_argument("--window-size=375,812")  # Mobile size simulation
        
        driver = selenium_webdriver.Chrome(options=chrome_options)
        logger.info("Desktop Chrome (Mobile Simulator viewport) driver initiated successfully.")
...
def generate_excel_report(results, start_time, end_time, duration, passed, failed):
    ...
    ws_summary["A1"] = "MOBILE APPIUM EXECUTION SUMMARY"
...
<head>
    ...
    <title>VoiceMail AI - Mobile Automation Report</title>
...
<body>
    <div class="container">
        <div class="header">
            <div>
                <h1>VoiceMail AI - Mobile E2E Report</h1>
                <p style="margin: 0.5rem 0 0 0; color: var(--text-muted);">Android Appium Automation Execution Dashboard</p>
            </div>
...
def generate_summary_markdown(results, base_url, passed, failed):
    ...
    repo = os.environ.get("GITHUB_REPOSITORY", "sathwikjuturu/voice-assistantce")
    username = repo.split('/')[0] if '/' in repo else "sathwikjuturu"
    repo_name = repo.split('/')[1] if '/' in repo else "voice-assistantce"
    
    report_url = f"https://{username}.github.io/{repo_name}/reports/latest/execution-report.html"
